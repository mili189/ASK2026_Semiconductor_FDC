...
1. 데이터셋 문단
- 데이터셋에 대한 설명을 절반으로 줄임 ✅
- 줄인 만큼의 분량을 전처리 방식에 대한 설명으로 추가 ✅

2. RCA 문단
- 현재 연구 단계에서 RCA라는 표현은 적합하지 않다고 판단되어 사용하지 않음 ✅ (RCA -> Fault Localization)
- 다만 Features에 대한 후보군을 3~5개를 제시하고 그 후보군에 임의로 조작된 Feature가 항상 포함되어 있다는 흐름으로 논문 내용을 수정 ✅

3. TRACE-GPT 모델 문단
- 논문 내용이 워낙 이해하기 쉽게 작성됨, 오히려 내용이 Scientific하지 못할 수 있음
- 수식이나 그림을 활용한 디테일을 추가하거나 더 Scientic한 서술을 추가하기
...
4. TRACE-GPT 모델 모식도
- 성재, 규순이 아이디어를 논의하여 피드백 후 수정


5. RCA 결과 그림
- Loss가 가장 큰 Feature 후보군으로 5개를 제시
- 정상과 불량 웨이퍼의 Loss를 비교하는 그래프 형태로 제작 예정
- 코드로 우선 구현 후, Raw Data 보내기
